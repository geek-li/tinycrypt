...
void chas_whiten(void *out, const void *in) {
  int i;
  for (i=0; i<16; i++) {
...
void chaskey(uint8_t *tag, uint32_t taglen, const uint8_t *m, const uint32_t mlen, const uint32_t k[4], const uint32_t k1[4], const uint32_t k2[4]) {
  ...
  const uint32_t *end = M + (((mlen-1)>>4)<<2); 
  /* pointer to last message block */
  ...
  // if blocks are padded by 16-bytes, use k1 
  if ((mlen != 0) && ((mlen & 0xF) == 0)) {
    l = k1;
    lastblock = M;
  } else {
    // else use k2 and add remaining plus 0x1 for padding
    l = k2;
    p = (uint8_t*) M;
    i = (m + mlen) - p;
