...
HashReturn Final(hashState *state, BitSequence *hashval)
{
      ...
      switch (state->hashbitlen) {
            case 224:  __movsb(hashval,state->H+100,28);  break;
            case 256:  __movsb(hashval,state->H+96, 32);  break;
            case 384:  __movsb(hashval,state->H+80, 48);  break;
            case 512:  __movsb(hashval,state->H+64, 64);  break;
      }
